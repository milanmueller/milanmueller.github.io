...
_Note: This is not a tutorial, it is a documentation of my personal experience of trying to learn the programming language [Haskell](https://www.haskell.org/)._
...
While being less intuitive than imperative languages, I found programming in a functional way to somehow be quite satisfying compared to writing for loops all the time.
Somehow the abstract way of thinking that functional programming necessitates tickles similar parts of my brain as videos of hydraulic presses crushing stuff do.
...
Now, actually calling what I'm planning to do a database might be a bit far fetched, so let's go with a more concrete description of my goal:
...
One could say that I have heard of some concepts from functional programming but I have no intuition for them.

### Personal Goals
* I want to gain an intuition for what monads are and what they can be used for (I'm just using monads as a representative example of a concept in Haskell that I want to learn).
...
* I will ask ChatGPT or gemini cli whenever I get stuck with something. I do think that there is something to gain from this approach even if it sometimes means to just do whatever the llm tells you.
...
Maybe this is due to oop to some degree, but I always feel like unless I find the perfect representation, class hierarchy, uml diagram or whatever right away, the project was going to end in an utter mess.
...
In oop languages this is already the point where I would maybe be wasting a lot of time thinking about class hierarchies and how to encapsulate this data properly but in Haskell I feel like it's very straight forward to come up with this.
...
From what I've heard, the `Either` type is a popular way of encapsulating potentially erronous states.
Another predefined type that might be useful to us here is the `State` type, since we are dealing with, well, some kind of state here.

This gives raise to the following monad stack (at least I hope that's what this thing is called):
``` Haskell
...
The entire source code can be inspected on [github](https://github.com/milanmueller/tasdb).
...
Of course I also hope that this little grammar could be extended in the future.
...
For each type of our AST, we create a parsing function. I won't include all of them here (the complete thing is on [github](https://github.com/milanmueller/tasdb)), but as an example, here is the parser for the types:
``` Haskell
...
There are no `if`s here, no `else if`s, no "_darn it, in this language it's `elif` not `else if`_"'s - life is functional - life is good.
...
## Run cabal, run!
Going from the AST we've now parsed to calling the functions that modify our state is what I will now refer to as _monkey pattern matching_ where we can just give a decently trained ape the type signature of our function and the monkey will then do the pattern matching for us.
One might also argue that instead of apes we could use llms for such tasks.

Since my cognitive ability is essentially equivalent to one of those "reasoning models" being used with a monkey brain for word prediction, I decided to implement functions like these myself:
``` Haskell
...
> metric add "os" enum ["linux", "evilbadcorp"]
OK
> value insert "os" "linux"
OK
> value insert "os" "evilbadcorp"
OK
> value insert "foo" "bar"
Error: Metric 'foo' not found
> value insert "os" "solaris"
Error: Invalid enum value 'solaris' for metric 'os'. Valid options: ["evilbadcorp","linux"]
> :show
Types:
("os",Enum [evilbadcorp, linux])
Values:
("os",['evilbadcorp' - at 2025-08-28 23:12:55.999177386 UTC,'linux' - at 2025-08-28 23:12:47.571831704 UTC])
```
